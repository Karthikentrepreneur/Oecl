        ...
        <div className="flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            {user && (
              <Link 
                to="/dashboard" 
                className={`nav-link font-medium text-black hover:text-kargon-red ${isActive("/dashboard") ? "text-kargon-red" : ""}`}
              >
                DASHBOARD
              </Link>
            )}
            {isAdmin && (
              <Link 
                to="/admin" 
                className={`nav-link font-medium text-black hover:text-kargon-red ${isActive("/admin") ? "text-kargon-red" : ""}`}
              >
                ADMIN
              </Link>
            )}
        ...
        <div className="md:hidden absolute top-full left-0 right-0 bg-white py-4 shadow-md animate-fade-in border-t">
          <div className="container mx-auto px-4">
            <nav className="flex flex-col space-y-4">
              ...
              <Link 
                to="/blog" 
                className={`font-medium text-black hover:text-kargon-red ${isActive("/blog") ? "text-kargon-red" : ""}`} 
                onClick={() => setIsMenuOpen(false)}
              >
                BLOGS
              </Link>
              ...
              {user && (
                <Link 
                  to="/dashboard" 
                  className={`font-medium text-black hover:text-kargon-red ${isActive("/dashboard") ? "text-kargon-red" : ""}`} 
                  onClick={() => setIsMenuOpen(false)}
                >
                  DASHBOARD
                </Link>
              ...
              {isAdmin && (
                <Link 
                  to="/admin" 
                  className={`font-medium text-black hover:text-kargon-red ${isActive("/admin") ? "text-kargon-red" : ""}`} 
                  onClick={() => setIsMenuOpen(false)}
                >
                  ADMIN
                </Link>
              ...
              {user ? (
                <div className="pt-2">
                  {isAdmin && (
                    <Link 
                      to="/admin" 
                      className="flex items-center text-black hover:text-kargon-red py-2" 
                      onClick={() => setIsMenuOpen(false)}
                    >
