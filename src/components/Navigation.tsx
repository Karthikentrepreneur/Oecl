      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            {user && <Link to="/dashboard" className={`nav-link font-medium ${isScrolled ? 'text-kargon-dark' : 'text-white'} hover:text-kargon-red ${isActive('/dashboard') ? 'active' : ''}`}>
                DASHBOARD
              </Link>}
            {isAdmin && <Link to="/admin" className={`nav-link font-medium ${isScrolled ? 'text-kargon-dark' : 'text-white'} hover:text-kargon-red ${isActive('/admin') ? 'active' : ''}`}>
                ADMIN
              </Link>}
          </nav>
          ...
          <div className="hidden md:flex items-center gap-4">
            {user ? <div className="flex items-center gap-2">
                ...
                <Button variant="ghost" className={`font-medium ${isScrolled ? 'text-kargon-dark hover:text-kargon-red' : 'text-white hover:text-white/80'} hover:bg-transparent`} onClick={() => signOut()}>
          ...
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="md:hidden p-2">
            {isMenuOpen ? <X className={isScrolled ? 'text-kargon-dark' : 'text-white'} size={24} /> : <Menu className={isScrolled ? 'text-kargon-dark' : 'text-white'} size={24} />}
          </button>
        </div>
      </div>

      {/* Mobile Navigation */}
      {isMenuOpen && <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-md py-4 animate-fade-in">
          <div className="container mx-auto px-4">
            <nav className="flex flex-col space-y-4">
              ...
              {user && <Link to="/dashboard" className={`font-medium text-kargon-dark hover:text-kargon-red py-2 ${isActive('/dashboard') ? 'text-kargon-red' : ''}`} onClick={() => setIsMenuOpen(false)}>
                  DASHBOARD
                </Link>}
              {isAdmin && <Link to="/admin" className={`font-medium text-kargon-dark hover:text-kargon-red py-2 ${isActive('/admin') ? 'text-kargon-red' : ''}`} onClick={() => setIsMenuOpen(false)}>
                  ADMIN
                </Link>}
              
              {user ? <div className="pt-2">
                  {isAdmin && <Link to="/admin" className="font-medium text-kargon-dark hover:text-kargon-red py-2 flex items-center" onClick={() => setIsMenuOpen(false)}>
                  ...
                  <button className="font-medium text-kargon-dark hover:text-kargon-red py-2 flex items-center" onClick={() => {
              setIsMenuOpen(false);
              signOut();
            }}>
