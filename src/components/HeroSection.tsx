// All previous import lines remain the same
    ...
    <section className="relative min-h-screen overflow-hidden bg-black text-white">
      ...
      <div className="absolute inset-0 z-10 overflow-hidden">
        {sliderImages.map((slide, i) => (
          ...
          >
            <img src={slide.url} alt={`Slide ${i}`} className="w-full h-full object-cover transition-transform duration-2000" />
        ...
        <div className="max-w-4xl space-y-8 px-0 py-0 mx-[200px]">
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl">
            {currentSlide.title.split(" ").map((word, i) => (
              <span key={i} className="text-slate-50 font-bold text-4xl" style={{ animationDelay: `${i * 0.1}s` }}>
                {word}{" "}
              </span>
            ))}
          </h1>
            ...
            {sliderImages.map((_, i) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 ${
                  activeSlide === i ? "bg-red-500 scale-125 shadow-lg shadow-red-500/50" : "bg-white/30 hover:bg-white/50"
                }`}
              />
            ))}
          </div>
              ...
              const ButtonContent = (
                <div className="group relative overflow-hidden w-full h-12 sm:h-14 flex flex-col gap-1 items-center justify-center text-xs transition-all duration-300 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 hover:-translate-y-1">
                  ...
                  <div className="relative z-10 flex flex-col items-center gap-0.5">
                    <div className="p-1.5 bg-white/20 rounded-lg group-hover:bg-white/30 transition-colors">{link.icon}</div>
            ...
            <div className="p-6 overflow-y-auto">
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {[{ src: "/GGL_demo1.mp4", label: "Getting Started", duration: "5:32" }, { src: "/GGL_promo.mp4", label: "Advanced Features", duration: "7:45" }].map((video, i) => (
                    <div key={i} className="group border-2 border-gray-200 rounded-xl overflow-hidden bg-gray-50 hover:border-red-300 transition-all duration-300 hover:shadow-lg">
                      <div className="aspect-video relative">
                        ...
                        <div className="absolute top-2 right-2 bg-black/70 text-white px-2 py-1 rounded text-xs">{video.duration}</div>
                      </div>
                      <div className="p-4">
                        <h4 className="font-semibold text-gray-800 group-hover:text-red-600 transition-colors">{video.label}</h4>
                        <p className="text-sm text-gray-600 mt-1">Learn how to use the portal effectively</p>
                      </div>
                    </div>
                  ))}
                </div>
