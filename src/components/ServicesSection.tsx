...
const getCountrySlugFromPath = (pathname: string): string => {
  const segments = pathname.split('/');
  const slug = segments[1]?.toLowerCase();

  const validCountries = ['india', 'malaysia', 'thailand', 'indonesia'];
  return validCountries.includes(slug) ? slug : 'india';
};
...
const ServicesSection = () => {
  ...
  const currentCountrySlug = getCountrySlugFromPath(location.pathname);

  // Redirect to country-specific page if missing
  useEffect(() => {
