...
const NewsDetailPage = () => {
    ...
    const processedLines = lines.map((line, index) => {
      for (const section of sections) {
        if (line.includes(section.title) && !processedSections.has(section.title)) {
          ...
          return <div key={`${section.title}-${index}`} className="flex items-center mt-6 mb-2">
  ...
  return <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-[158px]">
        ...
        {newsItem.imageUrl && <div className="w-full bg-white rounded-lg mb-8 p-4 flex justify-center items-center max-h-[400px]">
            <img src={newsItem.imageUrl} alt={newsItem.title} className="max-w-full max-h-[400px] object-contain" loading="lazy" />
          </div>}
        ...
        <Card className="mb-8 shadow-lg border border-gray-200 bg-white rounded-xl p-4">
          <CardHeader className="pb-2">
            ...
            <div className="flex flex-wrap items-center gap-4 mt-4 text-gray-600">
              ...
              {newsItem.category && <div className="flex items-center">
